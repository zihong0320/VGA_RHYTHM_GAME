...
* **핵심 기능**:
  ...
  * **Python UI 연동**: UART 통신을 통한 PC(Python UI)와 FPGA 간 실시간 데이터 인터페이스 구축
...
OV7670 카메라 설정 및 제어를 위해 SCCB 프로토콜을 구현하여 레지스터를 초기화합니다.
...
* 게임 전반의 상태(`IDLE`, `SELECT`, `READY`, `GAME`, `DONE`)를 관리하는 State Machine입니다.
...
* Line Counter의 노트 위치와 카메라/버튼 입력 신호를 비교하여 Perfect, Good, Miss 판정을 수행하고 점수를 업데이트합니다.
...
### 💡 프로젝트 고찰
* 팀원과의 명확한 포트, 방식의 정의에 대한 소통 없이 서브 모듈을 통합하는 과정조차 어려웠으며 소통의 중요성을 체감함.
